...
#  making list of length of countries name
for c in sort_countries :
    len_cnt.append(len(c));


#  appending countries with long name and short name to their respective list
for cn in sort_countries :
    if(max(len_cnt) == len(cn)):
        long_cnt.append(cn);
    elif (min(len_cnt) == len(cn)):
        short_cnt.append(cn);

#  printing countries with short and long names
print(f"short name countries {short_cnt}");
print(f"long names conutries {long_cnt}");
